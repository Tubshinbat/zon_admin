...
const News = (props) => {
  ...
  const handleExcel = async () => {
    ...
    fmData.append("file", excel);
    props.importExcel(fmData);
  };
  ...
  const [columns, setColumns] = useState([
    ...
    },
    {
      ...
      render: (text, record) => {
        return record.type.map((el) => <Tag color="blue"> {el} </Tag>);
      },
    },
    {
      dataIndex: "date",
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/costs/edit/${selectedRowKeys[0]}`);
    }
  };

  const handleDelete = () => {
    props.deleteMultCost(selectedRowKeys);
  };
      ...
      props.costs.length > 0 &&
        props.costs.map((el) => {
          ...
          el.date = moment(el.date)
            .utcOffset("+0800")
            .format("YYYY-MM-DD HH:mm:ss");
          el.type = el.type.map((el) => el.name);
  ...
  const exportExcel = async () => {
    const query = queryBuild();
    const response = await axios.get("costs/excel?" + query);
    let excelData = [];
    if (response) {
    ...
    cloneColumns.map((col) => {
      col.status === true && head.push(col.title);
    });
  ...
  return (
    <>
                ...
                <div className="datatable-header-tools">
                  <div className="datatable-actions">
                    <button
                      ...
                      onClick={() => history.push(`/costs/add`)}
                    >
                    ...
                    >
                      <i className="fa fa-file-excel"></i> Excel -ээр дата нэмэх
                    </button>
                    <button
      ...
      {/* Excel */}
      <Modal
        visible={visible && visible.excel}
        onCancel={() => handleCancel()}
        title="Excel дата оруулах"
        footer={[
          <Button key="back" onClick={() => handleCancel()}>
            Буцах
          </Button>,
      ...
      >
        <div className="modal__excel_body">
          <div className="excel__template">
            <a href="/assets/cost_template.xlsx"> Жишээ загвар татах </a>
          </div>
          <Upload
            name="excel"
            accept=".xlsx, .xls, .csv"
...
const mapStateToProps = (state) => {
  return {
    ...
    pagination: state.costReducer.paginationLast,
    excelData: state.costReducer.excelData,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    importExcel: (file) => dispatch(actions.importExcel(file)),
    loadCost: (query) => dispatch(actions.loadCost(query)),
    deleteMultCost: (ids) => dispatch(actions.deleteMultCost(ids)),
