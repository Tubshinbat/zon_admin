...
const Add = (props) => {
  ...
  useEffect(() => {
    if (props.member) {
      ...
      props.member.pictures &&
        props.member.pictures.length > 0 &&
        setPictures(
          props.member.pictures.map((img) => ({
            name: img,
            url: `${base.cdnUrl}${img}`,
          }))
        );

      if (props.member.type && props.member.type.length > 0) {
        setCheckedKeys(() => {
          return props.member.type.map((cat) => cat._id);
        });
      }
    }
  }, [props.member]);
