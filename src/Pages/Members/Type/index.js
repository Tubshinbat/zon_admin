...
// Actions
import * as actions from "../../../redux/actions/memberTypeActions";
...
const Types = (props) => {
  ...
  const editMenu = (values) => {
    props.updateMemberType(values, select[0]);
    handleCancel();
  };

  const deleteMenu = () => {
    props.deleteMemberType(select[0], selectData);
    setSelect([]);
    setSelectData({});
    handleCancel();
  };
  ...
  const showModal = (modal) => {
    switch (modal) {
      ...
      case "edit": {
        if (select && select.length === 1) {
          form.setFieldsValue(props.type);
          setVisible((sb) => ({ ...sb, [modal]: true }));
        } else {
          toastControl("error", "Нэг өгөгдөл сонгоно уу");
        }
...
const mapStateToProps = (state) => {
  return {
    success: state.memberTypeReducer.success,
    error: state.memberTypeReducer.error,
    loading: state.memberTypeReducer.loading,
    types: state.memberTypeReducer.types,
    type: state.memberTypeReducer.type,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    saveMemberType: (data) => dispatch(actions.saveMemberType(data)),
    loadMemberTypes: () => dispatch(actions.loadMemberTypes()),
    getMember: (id) => dispatch(actions.loadMemberType(id)),

    updateMemberType: (data, id) =>
      dispatch(actions.updateMemberType(data, id)),
    deleteMemberType: (id) => dispatch(actions.deleteMemberType(id)),
