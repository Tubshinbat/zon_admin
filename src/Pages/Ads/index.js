...
const Ads = (props) => {
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/adsies/edit/${selectedRowKeys[0]}`);
    }
  };

  const handleDelete = () => {
    props.deleteMultAds(selectedRowKeys);
  };
  ...
  const exportExcel = async () => {
    const query = queryBuild();
    const response = await axios.get("adsies/excel?" + query);
    let excelData = [];
    if (response) {
          ...
          <div className="container-fluid">
            <div className="card datatable-card">
              <div className="card-header">
                <h3 className="card-title">Бүх зарлал</h3>
              </div>
                ...
                <div className="datatable-header-tools">
                  <div className="datatable-actions">
                    <button
                      ...
                      onClick={() => history.push(`/adsies/add`)}
                    >
...
const mapStateToProps = (state) => {
  return {
    ...
    adsies: state.adsReducer.adsies,
    pagination: state.adsReducer.paginationLast,
    excelData: state.adsReducer.excelData,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    loadAds: (query) => dispatch(actions.loadAds(query)),
    deleteMultAds: (ids) => dispatch(actions.deleteMultAds(ids)),
