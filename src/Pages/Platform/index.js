...
const Platforms = (props) => {
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        return (
          <div className="table-image">
            {record.icon ? (
              <img src={`${base.cdnUrl}150x150/${record.icon}`} />
            ) : (
              "Дүрс оруулаагүй байна"
            )}
          </div>
        );
      },
  ...
  const handleEdit = () => {
    if (selectedRowKeys.length != 1) {
      ...
      history.push(`/platforms/edit/${selectedRowKeys[0]}`);
    }
  };

  const handleDelete = () => {
    props.deleteMultPlatform(selectedRowKeys);
  };
    ...
    if (props.platforms) {
      ...
      props.platforms.length > 0 &&
        props.platforms.map((el) => {
          ...
          el.isDirect =
            el.isDirect == true ? "Линк холбосон" : "Линк холбоогүй";
  ...
  const exportExcel = async () => {
    const query = queryBuild();
    const response = await axios.get("platforms/excel?" + query);
    let excelData = [];
    if (response) {
          ...
          <div className="container-fluid">
            <div className="card datatable-card">
              <div className="card-header">
                <h3 className="card-title">Бүх платформууд</h3>
              </div>
                ...
                <div className="datatable-header-tools">
                  <div className="datatable-actions">
                    <button
                      ...
                      onClick={() => history.push(`/platforms/add`)}
                    >
...
const mapStateToProps = (state) => {
  return {
    ...
    platforms: state.platformReducer.platforms,
    pagination: state.platformReducer.paginationLast,
    excelData: state.platformReducer.excelData,
  };
};

const mapDispatchToProp = (dispatch) => {
  return {
    loadPlatform: (query) => dispatch(actions.loadPlatform(query)),
    getPlatform: (id) => dispatch(actions.getPlatform(id)),
    deleteMultPlatform: (ids) => dispatch(actions.deleteMultPlatform(ids)),
    getExcelData: (query) => dispatch(actions.getExcelData(query)),
    clear: () => dispatch(actions.clear()),
  };
};

export default connect(mapStateToProps, mapDispatchToProp)(Platforms);
